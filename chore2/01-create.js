...
(async () => {
  ...
  const res = await sheets.spreadsheets.create({
    requestBody: {
      properties: { title: 'Kids Chore Chart & Allowance Tracker v2' },
      sheets: [
  ...
  for (const sid of Object.values(sheetMap)) {
    reqs.push({ repeatCell: { range: gridRange(sid,0,1000,0,26), cell: { userEnteredFormat: { backgroundColor: hex(C.white) } }, fields: 'userEnteredFormat.backgroundColor' } });
  }
